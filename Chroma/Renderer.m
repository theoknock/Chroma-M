...
@implementation Renderer
{
    ...
    id <MTLTexture> _colorMaps[10];
    const id <MTLTexture> * _colorMaps_t;
...
//                    });
        ...
        id<MTLFunction> computeKernel = [defaultLibrary newFunctionWithName:@"frameDifferencingKernel"];
        ...
        for (int i = 0; i < 10; i++) {
            MTLTextureDescriptor * descriptorP = [MTLTextureDescriptor
                                                  ...
                                                  mipmapped:FALSE];
            [descriptorP setUsage:MTLTextureUsageShaderWrite | MTLTextureUsageShaderRead | MTLTextureUsageRenderTarget];
            _colorMaps[i] = [_device newTextureWithDescriptor:descriptorP];
        }
        
        _colorMaps_t = &_colorMaps;
...
- (void)drawInMTKView:(nonnull MTKView *)view
{
    @autoreleasepool {
        ...
        [computeEncoder setTexture:_colorMapPrev
                           atIndex:2];
            [computeEncoder setTexture:*_colorMaps_t atIndex:3];
...
//            for (int i = 0; i < 10; i++) {
//                id<MTLBlitCommandEncoder> copier = [commandBuffer blitCommandEncoder];
//                [copier copyFromTexture:_colorMap toTexture:_colorMaps[color_map_index]];
//                [copier optimizeContentsForGPUAccess:_colorMaps[color_map_index]];
//                [copier endEncoding];
//            }
//            color_map_index = (color_map_index + 1) % 9;
        }
