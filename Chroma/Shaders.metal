...
[[visible]] matrix_half3x3 edges(half coefficient) {
...
[[visible]] matrix_half3x3 horizontal_axis_edge(half coefficient) {
...
[[visible]] matrix_half3x3 vertical_axis_edge(half coefficient) {
...
[[visible]] matrix_half3x3 emboss(half coefficient) {
...
[[visible]] matrix_half3x3 ridges(half coefficient) {
    matrix_half3x3 convolution_kernel = matrix_half3x3(-1, -1, -1,
                                                       -1,  8, -1,
                                                       -1, -1, -1);
    const matrix_half3x3 coefficient_matrix = matrix_half3x3(coefficient, coefficient, coefficient, coefficient, coefficient, coefficient, coefficient, coefficient, coefficient);
...
[[visible]] matrix_half3x3 gaussian_blur(half coefficient) {
...
[[visible]] matrix_half3x3 box_blur(half coefficient) {
...
[[visible]] matrix_half3x3 sharpen(half coefficient) {
...
[[visible]] matrix_half3x3 ridges_alt(half coefficient) {
    matrix_half3x3 convolution_kernel = matrix_half3x3(0,-1,0,-1,4,-1,0,-1,0);
    const matrix_half3x3 coefficient_matrix = matrix_half3x3(coefficient, coefficient, coefficient, coefficient, coefficient, coefficient, coefficient, coefficient, coefficient);
    matrix_half3x3 convolution = convolution_kernel * coefficient_matrix;
    return convolution;
...
computeKernel(
              ...
              const array<texture2d<half, access::read>, 10> inTextures [[texture(3)]],
              uint2                          gid        [[ thread_position_in_grid ]]
              )
{
//    for (size_t i = 0; i<inTextures.size(); i++) {
//        if (is_null_texture(inTextures[i])) break;
//        half4 color = inTextures[i].read(gid);
//        outTexture.write(color, gid);
//    }

    
    convolution3x3(inTexture, outTexture, gid, identity + emboss(1), uint2(0,0));
}
...
//    outTexture.write((half4(half3(abs((inputImageTextureP.rgb * (half3)(3.5001, 3.5001, 3.5001)) - (inputImageTexture.rgb * (half3)(3.4999, 3.4999, 3.4999)))), 1.0)), gid);
//    half4 diff = (half4(half3(median3(inputImageTextureP.r, inputImageTextureP.g, inputImageTextureP.b) -
//                       median3(inputImageTexture.r, inputImageTexture.g, inputImageTexture.b)), 1.0));
    half4 diff_ = abs((half4(half3((inputImageTextureP.b, inputImageTextureP.g, inputImageTextureP.r) -
                       (inputImageTexture.b, inputImageTexture.g, inputImageTexture.r)), 1.0))) ;
//    clamp(0.0, 1.0, abs(diff_));
    outTexture.write(half4(diff_.r, diff_.g, diff_.b, 1.0), gid);
}
